...
export interface IQSNetwork {
  ...
  readonly preconfiguredVpcCidrAccessRDSPostgressSecurityGroup: ec2.ISecurityGroup;
  readonly preconfiguredVpcCidrAccessRDSMysqlSecurityGroup: ec2.ISecurityGroup;
...
abstract class QSNetworkBase extends Construct implements IQSNetwork {
  ...
  public abstract readonly preconfiguredVpcCidrAccessRDSPostgressSecurityGroup: ec2.ISecurityGroup;
  public abstract readonly preconfiguredVpcCidrAccessRDSMysqlSecurityGroup: ec2.ISecurityGroup;
...
export class QSNetworkMain extends QSNetworkBase {
  ...
  public readonly preconfiguredVpcCidrAccessRDSPostgressSecurityGroup: ec2.ISecurityGroup;
  public readonly preconfiguredVpcCidrAccessRDSMysqlSecurityGroup: ec2.ISecurityGroup;
  ...
  public constructor(scope: Construct, id: string, props: QSNetworkProps) {
    ...
    this.preconfiguredNoAccessSecurityGroup = new ec2.SecurityGroup(
      this,
      `${props.stackName}-Default-NO-Access-SG`,
      {
        vpc: this.vpc,
    ...
    this.preconfiguredVpcCidrAccessHttpSecurityGroup = new ec2.SecurityGroup(
      this,
      `${props.stackName}-Default-HTTP-Access-SG`,
      {
        vpc: this.vpc,
    ...
    this.preconfiguredVpcCidrAccessHTTPSSecurityGroup = new ec2.SecurityGroup(
      this,
      `${props.stackName}-Default-HTTPS-Access-SG`,
      {
        vpc: this.vpc,
    ...
    this.preconfiguredVpcCidrAccessRDSPostgressSecurityGroup =
      new ec2.SecurityGroup(
        this,
        `${props.stackName}-Default-RDS-POSTGRESS-Access-SG`,
        {
          vpc: this.vpc,
          securityGroupName: `${props.stackName}-Default-RDS-POSTGRESS-Access-SG`,
          allowAllOutbound: true, //oubound access allowed - Inbound access allowed at 5432
          description: "Default-RDS-POSTGRESS-Access-SG",
        }
      );
    this.preconfiguredVpcCidrAccessRDSPostgressSecurityGroup.addIngressRule(
      ec2.Peer.ipv4(this.vpc.vpcCidrBlock),
      ec2.Port.tcp(5432),
      "Allow DB traffic from CIDR"
    );
    this.preconfiguredVpcCidrAccessRDSMysqlSecurityGroup =
      new ec2.SecurityGroup(
        this,
        `${props.stackName}-Default-RDS-MYSQL-Access-SG`,
        {
          vpc: this.vpc,
          securityGroupName: `${props.stackName}-Default-RDS-MYSQL-Access-SG`,
          allowAllOutbound: true, //oubound access allowed - Inbound access allowed at 3306
          description: "Default-RDS-MYSQL-Access-SG",
        }
      );
    this.preconfiguredVpcCidrAccessRDSMysqlSecurityGroup.addIngressRule(
      ec2.Peer.ipv4(this.vpc.vpcCidrBlock),
      ec2.Port.tcp(3306),
      "Allow DB traffic from CIDR"
    );
...
class LookedUpNetwork extends QSNetworkBase {
  ...
  constructor(scope: Construct, id: string, stackName: string, vpcId: string) {
    ...
    this.preconfiguredNoAccessSecurityGroup =
      ec2.SecurityGroup.fromLookupByName(
        this,
        `${stackName}-Default-NO-Access-SG`,
        `${stackName}-Default-NO-Access-SG`,
        this.vpc
      );
    this.preconfiguredVpcCidrAccessHttpSecurityGroup =
      ec2.SecurityGroup.fromLookupByName(
        this,
        `${stackName}-Default-HTTP-Access-SG`,
        `${stackName}-Default-HTTP-Access-SG`,
        this.vpc
      );
    this.preconfiguredVpcCidrAccessHTTPSSecurityGroup =
      ec2.SecurityGroup.fromLookupByName(
        this,
        `${stackName}-Default-HTTPS-Access-SG`,
        `${stackName}-Default-HTTPS-Access-SG`,
        this.vpc
      );
    ...
    this.preconfiguredVpcCidrAccessRDSPostgressSecurityGroup =
      ec2.SecurityGroup.fromLookupByName(
        this,
        `${stackName}-Default-RDS-POSTGRESS-Access-SG`,
        `${stackName}-Default-RDS-POSTGRESS-Access-SG`,
        this.vpc
      );
    this.preconfiguredVpcCidrAccessRDSMysqlSecurityGroup =
      ec2.SecurityGroup.fromLookupByName(
        this,
        `${stackName}-Default-RDS-MYSQL-Access-SG`,
        `${stackName}-Default-RDS-MYSQL-Access-SG`,
        this.vpc
      );
  }
